...
> 状态：🟩 **初版完成（S2.1–S2.3）** ｜ 步数预算 **9** ｜ 已用 **3** ｜ 认领人：无
...
## S2.3 补充（2026-09-03）：第二种 SCM 结构

| 产出 | 位置 |
|---|---|
| 三种信号形式 | `components/scm_generator.py` 的 `signal_form` |
| 非线性场景库 | `configs/scenarios_nonlinear.yaml`（6 场景） |
| 调参后的阶梯结果 | `modules/m5_modeling/results/nonlinear_ladder.csv` |

| 形式 | 信号 | 检验的机制 |
|---|---|---|
| `linear`（默认） | s_shared + s_a + λ·s_b | 可加贡献 |
| `interaction` | s_shared + s_a + λ·(s_a × s_b) | 被动方**只经交互**起作用 |
| `threshold` | s_shared + s_a + λ·1[s_b > 0] | 被动方以阶跃方式起作用 |

**默认必须保持 linear**——既有全部结论都建立在该形式上，默认值一旦改变会静默污染
历史结论。该不变量由 `test_默认形式为线性且向后兼容` 锁定。

### 🔴 分支卡 BC-M5-001 的 falsifier 触发

强交互场景 N2（λ=2.0）下模型排序反转：
...
三条 L3 路线的取舍改由**暴露面与合规成本**决定。

**三项验收**：①向后兼容——linear 形式下 S1_基准 的理论增益逐位等于此前记录的 0.028082；
②零互补——三种形式在 λ=0 时理论增益均为 0；
③配置口径——`N6_线性_对照` 与原场景库 `S1_基准` 的理论增益差 < 1e-12。
