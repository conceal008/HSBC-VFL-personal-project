...
> 状态：🟨 **规格级完成（S4.1），实现待真实数据** ｜ 步数预算 **10** ｜ 已用 **1** ｜ 认领人：无
> 最后更新：2026-09-04 ｜ 规范位置：`docs/00-framework/` §M4 · `docs/01-loops/` LOOP M4
...
[`待真实数据的特征工程规格.md`](待真实数据的特征工程规格.md)

**为什么不在合成数据上做**：合成数据的特征是 SCM 直接生成的干净连续变量——
无缺失、无重尾、无类别型、无业务含义。在其上做分箱、WOE、稳定性筛选，
会产出一套**看似完整、实则没有信息量**的产物。比起交付自我背书的假工作，
更有价值的是把「数据到位时必须做什么」写清楚。

**本项目特有的一步**：常规特征工程从「这个特征有没有预测力」开始，
本项目必须先问「**这个特征能不能用**」——F1–F4 的合规可用性筛查未完成前，
后续步骤全部不得开始。对一个最终不能用的字段做特征工程是纯浪费。

**规格里写下了三个已实测到的坑**：不要因冗余就剔除对方特征（冗余本身是价值来源）；
不要只用线性模型评估特征价值（会系统性漏掉交互型价值）；
不要试图用分段统计层面的特征工程替代联邦（L1 的结构性上界）。
