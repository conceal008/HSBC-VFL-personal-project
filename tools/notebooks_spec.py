...
def m7():
    build("modules/m7_security/notebooks/S7.1_privacy_attacks.ipynb", [
        ...
        ("md", "**旧的名单稳定性阈值 0.9 抓不到中等幅度的操纵**："
               ...
               "（零知识证明类手段），本阶段未实现。"),
        ("md", "## S7.5 · 模型窃取：资产还在不在\n\n"
               "前面的问题都是「数据有没有泄露」。这一节问的是另一个问题——"
               "**资产还在不在**。\n\n"
               "推导完全建立在已证的 A1 之上，不需要任何新假设：\n"
               "A1 已证被动方能完全推断标签 y → 由 `r = sigmoid(logit) − y` 反解出 logit →"
               "减去自己算得出的 `partial_b` → **得到主动方的全部贡献，逐样本**。"),
        ("code", "ms = pd.read_csv(ROOT / 'modules/m7_security/results/model_stealing.csv')\n"
                 "ms[ms.round_averaged].groupby('dp_sigma')[\n"
                 "    ['label_accuracy', 'score_correlation', 'topk_overlap']]"
                 ".mean().round(ROUND_DP)"),
        ("md", "**🔴 C12 · 一个有用的不对称。**\n\n"
               "σ=1.0 时标签推断准确率仍是 **1.0000**（C3 得到印证），"
               "但名单复现重合度已从 1.000 跌到 **0.126**（随机水平 0.10）。\n\n"
               "机制清楚：\n"
               "- **标签只需要残差的符号** —— 轮间恒定的一位信息，跨轮平均即可恢复\n"
               "- **模型窃取需要残差的精确数值** —— 随训练变化不可跨轮平均，"
               "且信号量级只有 O(正样本率)=0.03，被 σ=1.0 完全淹没\n\n"
               "→ **不要因为「噪声防不住标签」就不加噪声。** 它保的是另一样东西："
               "模型资产。而 σ=1.0 的可用性代价几乎为零（0.7869 vs 0.7868）。"),
        ("md", "### 加强攻击：平均收敛段的最后 M 轮"),
        ("code", "mt = pd.read_csv(ROOT / 'modules/m7_security/results/model_stealing_tail.csv')\n"
                 "mt.pivot_table(index='dp_sigma', columns='尾部平均轮数',\n"
                 "               values='名单重合', aggfunc='mean').round(ROUND_DP)"),
        ("md", "加强后 σ=1.0 时从 0.126 提到 0.408，仍远低于无防护的 1.000。"
               "**这不是攻击写弱了，是机制不同**——收敛段各轮的残差并不完全相同，"
               "平均会引入偏差，而噪声只按 1/√M 衰减。"),
        ("md", "### C13 · 多次建模的信息累积\n\n"
               "生产系统本来就要周期性重训，**每次重训都给攻击者一批全新的独立观测**。"
               "这与「重复探针」不同——重复探针会在流量上留下异常，"
               "而重训是协议要求的正常行为，攻击者只需要有耐心。"),
        ("code", "acc = pd.read_csv(ROOT / 'modules/m7_security/results/multi_run_accumulation.csv')\n"
                 "acc.groupby('重训次数')[['单次R2', '累积R2', '有效σ']].mean().round(ROUND_DP)"),
        ("md", "合法幅度 + σ=0.1 下，**6000 次重训（约每天重训 16 年）仍未攻破**。\n\n"
